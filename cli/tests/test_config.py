...
from mcp_toolkit.config import find_config_file, load_servers, to_stdio_params
...
args = []
cwd = "servers/alpha"
env = { TOKEN = "${MCP_TOOLKIT_TEST_TOKEN}", LITERAL = "no-substitution-here" }
"""
...
def test_load_servers_parses_all_entries(config_path):
...
def test_env_vars_are_expanded_from_the_cli_process_environment(config_path, monkeypatch):
    monkeypatch.setenv("MCP_TOOLKIT_TEST_TOKEN", "secret-value")
...
def test_missing_env_var_expands_to_empty_string(config_path, monkeypatch):
    monkeypatch.delenv("MCP_TOOLKIT_TEST_TOKEN", raising=False)
